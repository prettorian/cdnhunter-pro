...
# --- MÉTODO 2: CERTIFICATE TRANSPARENCY (CORREGIDO) ---
method_crt_sh() {
    local domain=$1
    ...
    # CORRECCIÓN AQUÍ: Se eliminó el espacio entre %. y $domain
    # Se usa wildcard *.domain.com para buscar subdominios
    local query="*.$domain"
    local json_data
    
    # Intentamos conectar con timeout de 20s
    json_data=$(curl -s --max-time 20 "https://crt.sh/?q=${query}&output=json" 2>/dev/null)
    
    if [ -z "$json_data" ] || [[ "$json_data" == *"error"* ]] || [[ "$json_data" == *"Value not permitted"* ]]; then
        echo -e "${RED}   Error al conectar con crt.sh o la API rechazó la consulta.${NC}"
        echo -e "${YELLOW}   Nota: crt.sh a veces limita consultas muy frecuentes.${NC}"
        return
    fi

    # Extraer nombres únicos, limpiar wildcards y filtrar el dominio base
    local subs
    subs=$(echo "$json_data" | jq -r '.[].name_value' 2>/dev/null | tr '\n' ' ' | sed 's/\*/./g' | tr ' ' '\n' | sort -u | grep -v "^$domain$" | grep "$domain" | head -n 25)
    
    if [ -z "$subs" ]; then
        echo -e "${GREEN}   No se encontraron subdominios públicos relevantes.${NC}"
        return
    fi
    ...
    while IFS= read -r sub; do
        [ -z "$sub" ] && continue
        # Evitar bucles infinitos si hay líneas vacías
        local clean_sub=$(echo "$sub" | xargs) 
        [ -z "$clean_sub" ] && continue

        local sub_ip
        sub_ip=$(dig +short "$clean_sub" A 2>/dev/null | head -n1)
        
        if [ -n "$sub_ip" ]; then
            local provider=$(is_cdn_ip "$sub_ip")
            if [ "$provider" == "Desconocido" ]; then
                echo -e "   ${GREEN}>> $clean_sub${NC} ($sub_ip) ${BOLD}[POSIBLE FUGA]${NC}"
            else
                echo -e "   $clean_sub ($sub_ip) [$provider]"
            fi
        else
            echo -e "   $clean_sub (Sin resolución IP)"
        fi
    done <<< "$subs"
}
...
method_hacker_target() {
    ...
    # Corrección menor: eliminar espacio en la URL si existiera
    local history
    ...
    echo "$history" | while IFS= read -r line; do
        if [[ $line =~ ^[0-9]+\.[0-9]+\.[0-9]+\.[0-9]+$ ]]; then
            ...
            # Ignorar líneas de cabecera o errores de texto
            : 
        fi
    done
}
